...
## nftables config
mv /tmp/kickstart/eth2nodes/opensuse/etc/sysconfig/nftables.conf /etc/sysconfig/nftables.conf
chown root:root /etc/sysconfig/nftables.conf
chmod 600 /etc/sysconfig/nftables.conf

## Sysctl tuning
cat /tmp/kickstart/eth2nodes/opensuse/etc/sysctl.d/99-sysctl-kvm.conf > /etc/sysctl.d/99-sysctl.conf
...
mv /tmp/kickstart/eth2nodes/opensuse/root/.profile-host /root/.profile
chown root:root /root/.profile
chmod 640 /root/.profile

# modprobe
mv /tmp/kickstart/eth2nodes/opensuse/etc/modules-load.d/zfs.conf /etc/modules-load.d/
chown -R root:root /etc/modules-load.d/
chmod 600 /etc/modules-load.d/

mv /tmp/kickstart/eth2nodes/opensuse/etc/modprobe.d/*.conf /etc/modprobe.d/
chown -R root:root /etc/modprobe.d
chmod 644 /etc/modprobe.d/*

# Enable virtual network bridges
mv /tmp/kickstart/eth2nodes/opensuse/etc/systemd/system/virt-nets.service /etc/systemd/system/

# Cleanup and enable services
rm -rf /tmp/kickstart
systemctl daemon-reload
systemctl enable fail2ban
systemctl enable auditd
systemctl enable nftables
systemctl enable virt-nets


# Enable ZFS - unsuported kernel module
sed -i "s/^allow_unsupported_modules.*/allow_unsupported_modules 1/g" /etc/modprobe.d/10-unsupported-modules.conf
