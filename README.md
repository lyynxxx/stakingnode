...
Because of that, I try to think carefully about the steps required to get a validator node, mostly secure and well configured. As time is money, I also try to do this as automated as possible, so that if anything goes wrong and I need to spin up a new machine to replace the old one, I could do it ASAP! It's mostly about my ETH2.0 validator setup, but the basics - the OS setup - are the same for almost any node.
