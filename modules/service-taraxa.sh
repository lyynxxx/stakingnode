...
## Copy the generated wallet/config file to /opt/taraxa/.conf and the binary to /opt/taraxa/bin
...
#su - taraxa -s /bin/bash -c "podman run -d -p 10002:10002 -p 10002:10002/udp --mount type=bind,source=/opt/taraxa/.conf,target=/opt/taraxa_data/conf --mount type=bind,source=/opt/taraxa/data,target=/opt/taraxa_data/data docker.io/taraxa/taraxa-node taraxad --network-id 2 --wallet /opt/taraxa_data/conf/wallet.json --config /opt/taraxa_data/conf/testnet.json --data-dir /opt/taraxa_data/data --overwrite-config"
